...
for idist = 1: length(dist)
    ...
    for ialpha = params.alpha
        ...
        for ibeta = params.beta
            ...
            for istim = 1:4
                accPlot{istim}(countalpha, countbeta) = meanAcc{countalpha, countbeta}(istim);
            end
...
% conditions 
figure(2);
C = colormap((cbrewer2('RdBu')));
titletext = {'LL-Safe:HH-Risky', 'LL-Safe:HHSafe', 'LL-Risky:HH-Safe', 'LLRisky:HHRisky'};
for istim = 1:4
    subplot(2, 2, istim)
    hold on
