function [best_fit_parm, LL, BIC] = mdlFit_RATES_riskPref(dataIn_all, params, dist, distSplit, nIters)

%-------------------------------------------------------------------
%%% RATES model fit to risk preferences 
%---------------------------------------------------------------------
%%% TRUE DATA NEEDED:
%%% 1) Stimulus LEFT 
%%% 2) Stimulus RIGHT 
...
if distSplit 
    ...
    obFunc = @(x) LL_RATES_riskPref(dataIn_all, x(1), x(2), x(3));
    LB = [0 0 0];
    UB = [1 1 3];
    % return set of parameters to be fit (alpha and beta for RW model)
    for iter = 1:nIters
        X0(iter,:) = [params.alpha_pos(iter) params.alpha_neg(iter) params.beta(iter)];
    end
